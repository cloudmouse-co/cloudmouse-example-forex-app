...
namespace ForexExample {
    ...
    ForexDisplayManager::ForexDisplayManager(ForexPreferences& prefs)
        : preferences(prefs)
        , currentScreen(ForexScreen::LOADING)
        , selectedSymbolIndex(0)
        , symbolCount(0)
        , screen_config_needed(nullptr)
        , screen_symbol_list(nullptr)
        , screen_symbol_detail(nullptr)
        , screen_loading(nullptr)
    ...
    bool ForexDisplayManager::init() {
        ...
        // Create encoder group for navigation
    ...
    void ForexDisplayManager::update() {
        // Process forex events from EventBus
        CloudMouse::Event sdkEvent;
        while (CloudMouse::EventBus::instance().receiveFromMain(sdkEvent, 0)) {
            // Convert SDK event back to ForexEventData
            // (Events with type >= 100 are forex events)
            if (static_cast<int>(sdkEvent.type) >= 100) {
                ForexEventData forexEvent;
                forexEvent.type = static_cast<ForexEventType>(static_cast<int>(sdkEvent.type) - 100);
            ...
            // Also handle encoder events for navigation
            if (sdkEvent.type == CloudMouse::EventType::ENCODER_ROTATION) {
                handleEncoderRotation(sdkEvent.value);
            } else if (sdkEvent.type == CloudMouse::EventType::ENCODER_CLICK) {
                handleEncoderClick();
            } else if (sdkEvent.type == CloudMouse::EventType::ENCODER_LONG_PRESS) {
    ...
    void ForexDisplayManager::handleEncoderRotation(int delta) {
        // LVGL encoder group handles this automatically for us!
        // Just make sure the active screen's objects are in the group
    }
